...
npx saddle deploy Unitroller "$RegistryProxy"
...
npx saddle deploy PTokenFactory "$RegistryProxy" 1000000000000000000 "$Unitroller" "$BaseInterestRateModel" 20000000000000000 100000000000000000
...
await unitroller1.methods._setLiquidateGuardian('0x6EBD066e2891922f3c1BfD426B2a49D5D0bc11C8').send({ from: "$from" })
await unitroller1.methods._setPauseGuardian('0x6EBD066e2891922f3c1BfD426B2a49D5D0bc11C8').send({ from: "$from" })
...
await registry1.methods._setOracle(uniswapPriceOracleProxy._address).send({ from: "$from" })
await pTokenFactory.methods._createPETH(pEtherDelegate._address).send()
await pTokenFactory.methods._createPPIE("0xb36afc9f38d8ac6f991bb9939d3ee8d45a7a1285", pPIEDelegate._address).send()