        ...
        if (eventArg === undefined) {
          ...
          try {
            ...
            if (arg.rescue) {
              ...
              // `Gate (PToken Eth Address) (... deploy pToken)`
              // could be used to deploy a pToken if it doesn't exist, but
              // since there is no PToken, that check would raise (when we'd
