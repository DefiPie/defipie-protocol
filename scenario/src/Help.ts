...
export function printHelp(printer: Printer, event: Event, expressions: Expression<any>[], path: string[]=[]) {
  if (event.length === 0) {
    ...
    if (path.length === 0) {
      banner = (
`
## DiFiPie Command Runner

The DiFiPie Command Runner makes it easy to interact with DiFiPie. You can input simple commands
...
sub-commands run \`Help PToken\` or \`Help PToken Mint\`.
`).trim();
    } else {
