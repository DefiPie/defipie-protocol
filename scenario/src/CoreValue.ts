...
import { getPieValue, pieFetchers } from './Value/PieValue';
...
const fetchers = [
    ...
    async (world, { addr, slot, key, nestedKey, valType }) => {
      ...
      switch (valType.val) {
        case 'marketStruct':
          ...
          let isPieStr = await world.web3.eth.getStorageAt(addr.val, isPieKey);

          return new ListV([
            new BoolV(isListed),
            new ExpNumberV(collateralFactor.toString(), 1e18),
            new BoolV(areEqual(userInMarket, 1)),
            new BoolV(areEqual(isPieStr, 1))
          ]);
        default:
          return new NothingV();
      }
  ...
  new Fetcher<{ given: Value; expected: Value }, BoolV>(
    `
      ...
      * "Equal given:<Value> expected:<Value>" - Returns true if given values are equal
        ...
        * E.g. "Equal (PToken pZRX TotalSupply) (Exactly 55)"
        * E.g. "Equal (PToken pZRX Controller) (Controller Address)"
    `,
    'Equal',
  ...
  new Fetcher<{ res: Value }, Value>(
    `
      ...
      * "PToken ...pTokenArgs" - Returns pToken value
    `,
    'PToken',
    [new Arg('res', getPTokenValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: pTokenFetchers() }
  ),
  new Fetcher<{ res: Value }, Value>(
    `
      #### PTokenDelegate

      * "PTokenDelegate ...pTokenDelegateArgs" - Returns pToken delegate value
    `,
    'PTokenDelegate',
    [new Arg('res', getPTokenDelegateValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: pTokenDelegateFetchers() }
  ),
  new Fetcher<{ res: Value }, Value>(
    `
      #### Erc20
  ...
  new Fetcher<{ res: Value }, Value>(
    `
      ...
      * "Pie ...pieArgs" - Returns Pie value
    `,
    'Pie',
    [new Arg('res', getPieValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: pieFetchers() }
  ),
  new Fetcher<{ res: Value }, Value>(
    `
      #### Governor
...
let contractFetchers = [
  ...
  { contract: "DeFiPieLens", implicit: false },
