...
import { pieCommands, processPieEvent } from './Event/PieEvent';
...
export const commands: (View<any> | ((world: World) => Promise<View<any>>))[] = [
    ...
    new View<{ res: Value }>(
      `
        ...
        * "Read ..." - Reads given value and prints result
          * E.g. "Read PToken pBAT ExchangeRateStored" - Returns exchange rate of pBAT
      `,
      'Read',
  ...
  new View<{ account: AddressV; event: EventV }>(
    `
      ...
      * "From <User> <Event>" - Runs event as the given user
        * E.g. "From Geoff (PToken pZRX Mint 5e18)"
    `,
    'From',
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "Trx ...trxEvent" - Handles event to set details of next transaction
        * E.g. "Trx Value 1.0e18 (PToken pETH Mint 1.0e18)"
    `,
    'Trx',
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "Invariant ...invariant" - Adds a new invariant to the world which is checked after each transaction
        * E.g. "Invariant Static (PToken pZRX TotalSupply)"
    `,
    'Invariant',
    [new Arg('event', getEventV, { variadic: true })],
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "Expect ...expectation" - Adds an expectation to hold after the next transaction
        * E.g. "Expect Changes (PToken pZRX TotalSupply) +10.0e18"
    `,
    'Expect',
    [new Arg('event', getEventV, { variadic: true })],
  ...
  new Command<{ given: Value; event: EventV }>(
    `
      ...
      * "Given value event" - Runs event only if value is truthy. Thus, given can be used to build existence checks.
        * E.g. "Given ($var) (PriceOracle SetPrice pBAT $var)"
    `,
    'Given',
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "PToken ...event" - Runs given PToken event
        * E.g. "PToken pZRX Mint 5e18"
    `,
    'PToken',
    [new Arg('event', getEventV, { variadic: true })],
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "PTokenDelegate ...event" - Runs given PTokenDelegate event
        * E.g. "PTokenDelegate Deploy CDaiDelegate cDaiDelegate"
    `,
    'PTokenDelegate',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => processPTokenDelegateEvent(world, event.val, from),
    { subExpressions: pTokenDelegateCommands() }
  ),
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "PriceOracle ...event" - Runs given Price Oracle event
        * E.g. "PriceOracle SetPrice pZRX 1.5"
    `,
    'PriceOracle',
    [new Arg('event', getEventV, { variadic: true })],
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * E.g. "PriceOracleProxy Deploy (Unitroller Address) (PriceOracle Address) (PToken pETH Address)"
    `,
    'PriceOracleProxy',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => {
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * E.g. "Maximillion Deploy (PToken pETH Address)"
    `,
    'Maximillion',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => {
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "Pie ...event" - Runs given pie event
      * E.g. "Pie Deploy"
    `,
    'Pie',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => {
      return processPieEvent(world, event.val, from);
    },
    { subExpressions: pieCommands() }
  ),
