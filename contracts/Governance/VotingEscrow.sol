...
contract VotingEscrow is VotingEscrowStorageV1, ReentrancyGuard, IVotingEscrow {
    ...
    mapping(address => mapping(uint => Point)) public override userPointHistory; // user -> Point[user_epoch]
    mapping(address => uint) public override userPointEpoch;
    mapping(uint => int128) public override slopeChanges; // time -> signed slope change
    ...
    function getPriorVotes(address user, uint blockNumber) external view returns (uint) {
        ...
        uint dLength = delegateAt[user].length;
        if (dLength == 0) {
            return balanceOfAtInternal(user, blockNumber);
        } else {
            uint votesAmount;
            for(uint i = 0; i < dLength;) {
