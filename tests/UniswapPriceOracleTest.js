...
describe('UniswapPriceOracle', () => {
    ...
    beforeEach(async () => {
        ...
        pEth = await makePToken({kind: "pether", controllerOpts: {kind: "bool"}, supportMarket: true});
        pOther = await makePToken({controller: pEth.controller, supportMarket: true});
        otherAddress = await call(pOther, "underlying");

        registryProxy = await call(pEth, "registry");
        ...
        await send(mockPriceFeed, 'setBlockTimestampLast', [
            block.timestamp
        ]);
    ...
    describe("check update function", () => {
        it("Check update function", async () => {
            ...
            let timestamp = +data.blockTimestampPrevious + +period;
            ...
            let otherInEth = new BigNumber('2785485648324132'); // 1 eth = 366,7647 other
        ...
        it("New pool", async () => {
            ...
            let block = await saddle.web3.eth.getBlock("latest");
            await send(newMockPriceFeed, 'setBlockTimestampLast', [
                block.timestamp
            ]);
    ...
    describe("constructor", () => {
        it("gets address of registry", async () => {
            ...
            expect(registryProxyAddress).toEqual(registryProxy);
        });
