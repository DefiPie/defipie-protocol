...
describe('Controller', () => {
  ...
  describe('liquidity', () => {
    ...
    it("allows entering 3 markets, supplying to 2 and borrowing up to collateralFactor in the 3rd", async () => {
      ...
      const pToken1 = await makePToken({supportMarket: true, collateralFactor: cf1, underlyingPrice: up1});
      const pToken2 = await makePToken({supportMarket: true, controller: pToken1.controller, collateralFactor: cf2, underlyingPrice: up2});
      const pToken3 = await makePToken({supportMarket: true, controller: pToken1.controller, collateralFactor: cf3, underlyingPrice: up3});
