...
describe.skip('PieWheelFuzzTest', () => {
  // This whole test is fake, but we're testing to see if our equations match reality.

  // First, we're going to build a simple simulator of the DeFiPie protocol
  ...
  let initialState = globals => {
    return {
      ...
      // flywheel & controller
      pieSupplySpeed: new bn(1),
      pieSupplyIndex: new bn(1),
      pieSupplyIndexSnapshots: {},
      pieSupplyIndexUpdatedBlock: globals.blockNumber,

      pieBorrowSpeed: new bn(1),
      pieBorrowIndex: new bn(1),
      pieBorrowIndexSnapshots: {},
      pieSupplyIndexUpdatedBlock: globals.blockNumber,

      pieAccruedWithCrank: {}, // naive method, accruing all accounts every block
      pieAccruedWithIndex: {}, // with indices

      activeBorrowBlocks: new bn(0), // # blocks with an active borrow, for which we expect to see pie distributed. just for fuzz testing.
  ...
  let trueUpPie = (globals, state) => {
    state = accrueInterest(globals, state);

    state = Object.keys(state.pieSupplyIndexSnapshots).reduce(
      (acc, account) => supplierFlywheelByIndex(globals, state, account),
      state
    );

    state = Object.keys(state.pieBorrowIndexSnapshots).reduce(
  ...
  // manual flywheel loops through every account and updates pie accrued mapping
  ...
  let flywheelByCrank = (
    ...
    for (let [account, balance] of Object.entries(balances)) {
      if (isPositive(totalSupply)) {
        pieAccruedWithCrank[account] = get(
          state.pieAccruedWithCrank[account]
        ).plus(
          deltaBlocks
    ...
    return {
      ...state,
      pieAccruedWithCrank: pieAccruedWithCrank,
    };
  };

  // real deal pie index flywheel™️
  let borrowerFlywheelByIndex = (globals, state, account) => {
    ...
    let indexSnapshot = pieBorrowIndexSnapshots[account];

    if (
      indexSnapshot !== undefined &&
      pieBorrowIndex.isGreaterThan(indexSnapshot) &&
    ...
    return {
      ...state,
      pieBorrowIndexUpdatedBlock: globals.blockNumber,
      pieBorrowIndex: pieBorrowIndex,
      pieBorrowIndexSnapshots: {
        ...state.pieBorrowIndexSnapshots,
        [account]: pieBorrowIndex
      }
    };
  };
  ...
  let supplierFlywheelByIndex = (globals, state, account) => {
    let {
      ...
      pieAccruedWithIndex,
      totalSupply,
      pieSupplyIndexUpdatedBlock
    } = state;

    let deltaBlocks = globals.blockNumber.minus(pieSupplyIndexUpdatedBlock);
    ...
    let indexSnapshot = pieSupplyIndexSnapshots[account];
    if (indexSnapshot !== undefined) {
      // if had prev snapshot,  accrue some pie
      pieAccruedWithIndex[account] = get(pieAccruedWithIndex[account]).plus(
        balances[account].times(pieSupplyIndex.minus(indexSnapshot))
      );
    }
    ...
    return {
      ...state,
      pieSupplyIndexUpdatedBlock: globals.blockNumber,
      pieSupplyIndex: pieSupplyIndex,
      pieSupplyIndexSnapshots: {
        ...state.pieSupplyIndexSnapshots,
        [account]: pieSupplyIndex
      },
      pieAccruedWithIndex: pieAccruedWithIndex
    };
  };
  ...
  // assert amount distributed by the crank is expected, that it equals # blocks with a supply * pie speed
  let crankCorrectnessInvariant = (globals, state, events, invariantFn) => {
    let expected = state.activeSupplyBlocks
      .times(state.pieSupplySpeed)
      .plus(state.activeBorrowBlocks.times(state.pieBorrowSpeed));

    let actual = Object.values(state.pieAccruedWithCrank).reduce(
    ...
    invariantFn(
      ...
      expected,
      actual,
      `crank method distributed pie inaccurately`
    );
  };

  // assert pie distributed by index is the same as amount distributed by crank
  let indexCorrectnessInvariant = (globals, state, events, invariantFn) => {
    let expected = state.pieAccruedWithCrank;
    let actual = state.pieAccruedWithIndex;
    invariantFn(
      (expected, actual) => {
  ...
  let runEvents = (globals, initState, events) => {
    ...
    return trueUpPie(globals, state);
  };
